...
class calculator:
    def __init__(self):
        window = Tk()
        window.title('MS calculator')
        window.geometry("425x400")
        window.configure(background="#9609ce")
        self.string = StringVar()
        entry = Entry(window, textvariable=self.string,width=53)
        ...
        for txt in values:
            ...
            if (txt == '='):
                ...
                btn.configure(background="#23ce09")
                ...
                btn.grid(row=row, column=col, padx=1, pady=1)
                btn.configure(background="grey",fg="white",)
